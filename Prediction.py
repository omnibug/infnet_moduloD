...
plt.grid(b=True, which='both', color='0.7',linestyle='-', axis='y')
plt.xticks(range(len(indices)), indices, rotation=90 )
plt.bar(range(len(indices)), importances[0][indices], color="#4169E1", align="center",edgecolor="#4169E1")
plt.xlabel(u"Atributos")
plt.ylabel(u"Importância")
plt.savefig(test_nbr+'_'+lmodel+'.png', dpi=200)
plt.show()
...
var_model = {'max_depth':2,'n_estimators':60000,'objective':'reg:linear','learning_rate':0.001,'min_child_weight':1, 'gamma':0.0, 'subsample':0.1, 'colsample_bytree':0.5, 'missing':np.nan,'silent':False }
